<template>
  <form>
    <div>
      <label for="type" class="div7">投稿タイプ:</label>
      <select id="type" v-model="type" class="div7">
    ...
    <div>
      <label for="genre" class="div7">ジャンル:</label>
      <select id="genre" v-model="genre" class="div7">
        <option value="null">選択してください</option>
    ...
    <div>
      <label for="prefecture" class="div7">都道府県:</label>
      <select id="prefecture" v-model="prefecture" class="div7">
    ...
    <div>
      ...
      <input type="text" id="name" v-model="name" style="background-color: white; border-bottom:solid 2px black; font-size: 150%;" placeholder="名前を入力" pattern=".{1,10}" />
      <p v-if="nameError" style="color: red;">名前は1文字以上10文字以下にしてください</p>
    </div>

    <div>
      <label for="content"></label>
      <textarea id="content" v-model="content" style="background-color: white; width: 100vw; font-size: 150%;" placeholder="投稿内容を入力" pattern=".{1,200}"></textarea>
      <p v-if="contentError" style="color: red;">投稿内容は1文字以上200文字以下にしてください</p>
    </div>

    <!-- ボタンがクリックされたときにhandleClickメソッドを実行する -->
    <button type="button" @click="handleClick" class="div7">送信</button>
  </form>
</template>
